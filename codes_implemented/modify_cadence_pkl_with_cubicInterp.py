            ...
            if step_rm_rows == 'random':
                ...
                if np_lc.shape[0] >= 4:
                    try:
                        ...
                        func_interp = scipy.interpolate.interp1d(np_lc[:,0], np_lc[:,1], kind='cubic', bounds_error=False, fill_value=0.)
                        func_interp_err = scipy.interpolate.interp1d(np_lc[:,0], np_lc[:,2], kind='cubic', bounds_error=False, fill_value=0.)
                        ...
                        #print('We have a light curve with similar MJD')
                        list_mjd = list(np_lc[:,0])
                        dup = [i for i, x in enumerate(list_mjd) if i != list_mjd.index(x)]
                        mask = np.ones(np_lc.shape, dtype=bool)
                        mask[dup] = False
                        result = np_lc[mask]
                        np_lc_filtered = result.reshape(result.shape[0]//np_lc.shape[1], np_lc.shape[1])

                        if np_lc_filtered.shape[0] >= 4:
                            firts_mjd = np_lc_filtered[:,0][0]
                            maximum = np.max(np_lc_filtered[:,0])

                            kde_load_model = joblib.load('./models_kde/model_{}_kde.joblib'.format(type_cadence))
                            obs_cadence = kde_load_model.sample(np_lc_filtered.shape[0]-1)
                            obs_cadence[obs_cadence < 0] = t_value           

                            func_interp = scipy.interpolate.interp1d(np_lc_filtered[:,0], np_lc_filtered[:,1], kind='cubic', bounds_error=False, fill_value=0.)
                            func_interp_err = scipy.interpolate.interp1d(np_lc_filtered[:,0], np_lc_filtered[:,2], kind='cubic', bounds_error=False, fill_value=0.)

                            mjd_modified = np.add.accumulate(obs_cadence.flatten()) + np.array([firts_mjd])
                            mjd_modified = mjd_modified[mjd_modified < maximum]

                            np_lc_aux = np.zeros(shape=(mjd_modified.shape[0] + 1, np_lc_filtered.shape[1]))
                            np_lc_aux[:,0] = np.concatenate((np.array([firts_mjd]), mjd_modified))
                            np_lc_aux[:,1] = func_interp(np_lc_aux[:,0])
                            np_lc_aux[:,2] = func_interp_err(np_lc_aux[:,0])
                            np_lc = np.copy(np_lc_aux)
                    ...
                    ## If there is less than 4 points in the light curve
                    #try:
                    #    func_interp = scipy.interpolate.interp1d(np_lc[:,0], np_lc[:,1], kind='cubic', bounds_error=False, fill_value=0.)
                    #    func_interp_err = scipy.interpolate.interp1d(np_lc[:,0], np_lc[:,2], kind='cubic', bounds_error=False, fill_value=0.)
                    #    obs_cadence[obs_cadence < 0] = median           
                    #    mjd_modified = np.add.accumulate(obs_cadence.flatten()) + np.array([firts_mjd])
                    #    np_lc[:,0] = np.concatenate((np.array([firts_mjd]), mjd_modified))
                    #    np_lc[:,1] = func_interp(np_lc[:,0])
                    #    np_lc[:,2] = func_interp_err(np_lc[:,0])
#
                    #except ValueError:
#
                    #    try:
...
#
                ...
                #print(np_lc)
                    ...
                    print('Light curve has less than 4 observations.')
...
if __name__ == '__main__':
    ...
    start = time.time()
    modified_cadence(name_original_data='kepler', step_rm_rows='kde', 
                     name_new_data='kepler_alcock_cubic',
                     type_cadence='alcock')
    end = time.time()
