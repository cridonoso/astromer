...
def create_classifier(pretrain_weights, window_size, n_classes, clf_name='mlp_att'):
    ...
    inp_placeholder = {
        'values': Input(shape=(window_size, 1), batch_size=None, name='values'),
        'times' : Input(shape=(window_size, 1), batch_size=None, name='times'),
        'mask'  : Input(shape=(window_size, 1), batch_size=None, name='mask')
    }
...
class CustomModel(Model):
    ...
    def test_step(self, data):
        ...
        self.compute_loss(y=y['label'], y_pred=y_pred)
        for metric in self.metrics:
            if metric.name != "loss":
                metric.update_state(y_true=y['label'], y_pred=y_pred)
...
def run(opt):
    ...
    clf_model = create_classifier(opt.pre_weights, opt.ws, num_cls, clf_name='mlp_all')
...
if __name__ == '__main__':
    ...
    parser.add_argument('--p', default='./presentation/experiments/astromer_2/results/nsp_adamw_factor/1_4_64_rmse_0.5/classification/test', type=str,
