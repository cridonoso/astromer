...
def sweep_train(config=None):
    with wandb.init(config=config):
        config = wandb.config
        title = '{}.{}.{}.{}.{:.2f}.{}.{:.2f}'.format(config.n_layers,
                                     config.n_heads,
                                     config.head_dim,
                                     config.dff,
                                     config.dropout_rate,
        ...
        astromer.fit(trainloader, 
                     ...
                     callbacks=[WandbModelCheckpoint(filepath=os.path.join(WEIGHTS_FOLDER, title, 'model'),
                                                     monitor='val_loss',
                                                     save_freq='epoch',
