...
def main():
    ...
    wandb.log({'n_heads':n_heads})
    wandb.log({'head_dim':head_dim})
    wandb.log({'n_layers':n_layers})
    wandb.log({'dff':dff})
    ...
    for subset in ['train', 'val']:
        num_cls = pd.read_csv(
            os.path.join(f'./data/records/alcock/fold_{fold_n}/alcock_20/', 'objects.csv')).shape[0]

        data[subset] = pretraining_pipeline(
            f'./data/records/alcock/fold_{fold_n}/alcock_20/{subset}',
