...
def build_lstm(maxlen, n_classes):
    ...
    serie  = Input(shape=(maxlen+3, 1), batch_size=None, name='input')
    times  = Input(shape=(maxlen+3, 1), batch_size=None, name='times')
    mask   = Input(shape=(maxlen+3, 1), batch_size=None, name='mask')

    placeholder = {'input':serie,
                   'mask_in':mask,
                   'times':times}

    m = tf.cast(placeholder['mask_in'][...,0], tf.bool)
...
def build_lstm_att(astromer, maxlen, n_classes, train_astromer=False):
    serie   = Input(shape=(maxlen+3, 1), batch_size=None, name='input')
    times   = Input(shape=(maxlen+3, 1), batch_size=None, name='times')
    mask_in = Input(shape=(maxlen+3, 1), batch_size=None, name='mask_in')
    mask    = Input(shape=(maxlen+3, 1), batch_size=None, name='mask')
    ...
    mask = tf.cast(1.-placeholder['mask'][...,0], dtype=tf.bool)
...
def build_mlp_cls(astromer, maxlen, n_classes, train_astromer=False):
    serie   = Input(shape=(maxlen+3, 1), batch_size=None, name='input')
    times   = Input(shape=(maxlen+3, 1), batch_size=None, name='times')
    mask_in = Input(shape=(maxlen+3, 1), batch_size=None, name='mask_in')
    mask    = Input(shape=(maxlen+3, 1), batch_size=None, name='mask')
    print('BUILDING MLP + CLS')
    ...
    x = LayerNormalization()(x)
    x = Dense(n_classes)(x)
    return Model(inputs=placeholder, outputs=x, name="FCCLS")
...
def build_mlp_att_cls(astromer, maxlen, n_classes, train_astromer=False):
    ...
    x_rest = x * mask
    x_rest = tf.reduce_sum(x_rest, 1)/tf.reduce_sum(mask, 1)
    x_cls = tf.slice(x, [0,0,0], [-1,1,-1])
    x_cls = tf.reshape(x_cls, [tf.shape(x_cls)[0], tf.shape(x_cls)[-1]])

    x = tf.concat([x_rest, x_cls], 1)
    x = Dense(1024, activation='relu')(x)
    x = Dense(512, activation='relu')(x)
    x = Dense(256, activation='relu')(x)
    x = LayerNormalization()(x)
    x = Dense(n_classes)(x)
    return Model(inputs=placeholder, outputs=x, name="FCATTCLS")
...
def get_classifier_by_name(name, config, astromer=None, train_astromer=False):
    ...
    if name == 'mlp_att':
        clf_model= build_mlp_att(astromer,
                                 ...
                                 train_astromer=train_astromer)
    if name == 'mlp_cls':
        clf_model= build_mlp_cls(astromer,
                                     config['astromer']['window_size'],
                                     num_cls,
                                     train_astromer=train_astromer)
    if name == 'mlp_att_cls':
