...
def divide_training_subset(frame, train_frac=None, val_frac=None, test_frac=None, test_meta=None, val_meta=None):
    """
    ...
    Args:
        ...
        train_frac (float): train fraction
        test_frac (float): test fraction, not required if given test_meta
        val_frac (float): validation fraction, not required if given val_meta
        test_meta (Dataframe): Dataframe following the astro-standard format containing test data
        val_meta (Dataframe): Dataframe following the astro-standard format containing val data
    Returns:
        tuple x3 : (name of subset, subframe with metadata)
    """
    assert (test_frac==None and test_meta==None), 'test_frac and test_meta cannot be None together'
        

    assert (val_frac==None and val_meta==None), 'val_frac and val_meta cannot be None tegether'
     
    if val_frac is not None: # assuming that val fraction is always available i.e., train or test fraction won't be given without val fraction
    ...
    if test_meta is not None and val_meta is None: #test subset and val fraction available
        sub_test = test_meta
        sub_train = frame.iloc[:n_train]
        sub_val   = frame.iloc[n_train:n_train+n_val]        

    elif test_meta is None and val_meta is None: # test and val fraction available
        sub_train = frame.iloc[:n_train]
        sub_val   = frame.iloc[n_train:n_train+n_val]
        sub_test  = frame.iloc[n_train+n_val:n_train+n_val+n_test]
    
    elif test_meta is not None and val_meta is not None: # test and val subsets available
        sub_train = frame # assuming that frame will just be train data only (independent of test and val data)
        sub_val = val_meta
        sub_test = test_meta
...
def create_dataset(meta_df,
    ...
    fracs[:len(subsets_frac)] = subsets_frac
