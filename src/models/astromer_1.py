...
def get_ASTROMER(num_layers=2,
    ...
    outputs = {'magnitudes': x}
    return Model(inputs=placeholder, outputs=outputs, name="ASTROMER-1")
...
class CustomModel(Model):
    ...
    def train_step(self, data):
        ...
        with tf.GradientTape() as tape:
            ...
            print(y['probed_mask'])
            loss = self.compute_loss(y=y['magnitudes'], 
                                     y_pred=y_pred['magnitudes'],
                                     sample_weight=y['probed_mask'])
        ...
        for metric in self.metrics:
            if metric.name != "loss":
                metric.update_state(y['magnitudes'], 
                                    y_pred['magnitudes'], 
                                    y['probed_mask'])
    ...
    def test_step(self, data):
        ...
        self.compute_loss(y=y, y_pred=y_pred, sample_weight=y['probed_mask'])
        for metric in self.metrics:
            if metric.name != "loss":
